...
const PaperViewer: React.FC<PaperViewerProps> = ({ isOpen, onClose, paper }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-7xl h-[92vh] md:h-[90vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="px-4 md:px-6 py-3 md:py-4 border-b">
          <div className="flex items-start justify-between">
            <div className="flex-1 pr-4">
              <DialogTitle className="text-base md:text-xl font-bold mb-1 break-words leading-snug">
                {paper.title}
              </DialogTitle>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          <div className="px-4 md:px-6 border-b overflow-x-auto">
            <TabsList className="h-auto">
              <TabsTrigger value="viewer" className="text-xs md:text-sm">View Paper</TabsTrigger>
              <TabsTrigger value="ai-help" className="text-xs md:text-sm">
